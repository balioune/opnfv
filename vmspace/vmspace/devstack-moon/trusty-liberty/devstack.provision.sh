...
echo 	\* Suppression de Python-six
apt-get remove -y python-six

# Build log directory
echo 	\* Création du repertoire des log Moon
mkdir /var/log/moon/
chown vagrant /var/log/moon/

# Modify /etc/keystone/keystone-paste.ini
echo \* Configuration /etc/keystone/keystone-paste.ini
...
echo 	\* Configuration de /etc/keystone/keystone.conf
...
echo 	\* Installation des policies par défauts
...
echo 	\* Construction de Moon ...
ln -s /opt/stack/keystone-orig/.git /opt/stack/keystone/
cd /opt/stack/keystone-moon/
...
# install moonclient
echo \* Installation de Moon client
cd /opt/stack/moonclient
python setup.py install

# Restart Apache
echo \* Redémarage de Apache
service apache2 restart

# configure moonclient
# moon intraxtension init
